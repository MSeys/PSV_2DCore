...
int init_ime_dialog(char *title, const char *initial_text, int max_text_length, int type, int option)
{
    ...
    utf8_to_utf16(reinterpret_cast<uint8_t *>(title), ime_title_utf16);
...
char* VitaKeyboardGet(const std::string& title, const std::string& initial_text, int max_len, int multiline)
{
    char *name = nullptr;
    ...
    if (multiline) 
	{
        init_ime_dialog((char*) title.c_str(), (const char*) initial_text.c_str(), max_len, SCE_IME_DIALOG_TEXTBOX_MODE_DEFAULT, SCE_IME_OPTION_MULTILINE);
    }

	else 
	{
        init_ime_dialog((char*) title.c_str(), (const char*) initial_text.c_str(), max_len, SCE_IME_DIALOG_TEXTBOX_MODE_DEFAULT, 0);
    }
    ...
    while (!done) {
	    ...
	    const int ime_result = update_ime_dialog();
        if (ime_result == IME_DIALOG_RESULT_FINISHED) 
		{
            name = reinterpret_cast<char*>(get_ime_dialog_input_text_utf8());
        }
		...
		{
			name = const_cast<char*>(initial_text.c_str());
		}
